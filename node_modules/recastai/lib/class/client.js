...
var Client = function () {
  ...
  _createClass(Client, [{
    ...
    value: function textConverse(text, options) {
      ...
      var request = agent('POST', _constants2.default.CONVERSE_ENDPOINT).set('Authorization', 'Token ' + token);
      if (proxy) {
        request.proxy(proxy);
      }

      return new Promise(function (resolve, reject) {
        request.send(data).then(function (res) {
          return resolve(new _conversation2.default(res.body.results));
        }).catch(function (err) {
    ...
    value: function textRequest(text, options) {
      ...
      var request = agent('POST', _constants2.default.REQUEST_ENDPOINT).set('Authorization', 'Token ' + token);
      if (proxy) {
        request.proxy(proxy);
      }
      ...
      return new Promise(function (resolve, reject) {
        request.send(data).then(function (res) {
          return resolve(new _response2.default(res.body.results));
        }).catch(function (err) {
          return reject(new _error2.default(err.message));
        });
      });
    ...
    value: function fileRequest(file, options) {
      ...
      return new Promise(function (resolve, reject) {
        request.attach('voice', file).send().then(function (res) {
          return resolve(new _response2.default(res.body.results));
        }).catch(function (err) {
          return reject(new _error2.default(err.message));
        });
